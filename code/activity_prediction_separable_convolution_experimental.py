...
class Classifier(nn.Module):
    def __init__(self, raw_ni, no, drop=.5):
        ...
        self.raw = nn.Sequential(
            SepConv1d(raw_ni,  32, 3, 2, 3, drop=drop),
            SepConv1d(    32,  64, 3, 4, 2, drop=drop),
            SepConv1d(    64, 128, 3, 4, 2, drop=drop),
            SepConv1d(   128, 255, 3, 4, 2),
            Flatten(),
            nn.Dropout(drop), nn.Linear(255, 64), nn.ReLU(inplace=True),
            nn.Dropout(drop), nn.Linear( 64, 64), nn.Sigmoid(inplace=True))

        self.out = nn.Sequential(
            nn.Linear(64, 128), nn.ReLU(inplace=True), nn.Linear(128, no))
