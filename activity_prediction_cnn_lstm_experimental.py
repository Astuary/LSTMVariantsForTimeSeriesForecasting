...
class CNN_LSTM(nn.Module):
    ...
    def forward(self, x):
        x = self.mp1(self.dp1(self.nl1(self.con1(x))))
        x = self.mp2(self.dp2(self.nl2(self.con2(x))))
        x = self.f1(x)
        x = self.lstm_layer1(x)
        x = self.nl3(self.l1(self.dp3(x[0])))
        x = self.lstm_layer2(x)
        x = self.sig(self.l2(self.dp4(x[0])))
        return x
